...
LONG WINAPI CrashHandler::vectoredExceptionHandler(EXCEPTION_POINTERS* exceptionInfo)
{
    ...
    if (exceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION
        ...
        if (exceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_STACK_OVERFLOW) {
            ...
            HANDLE dumpThread = CreateThread(
                nullptr, 0,
                [](LPVOID param) -> DWORD {
                    auto* exceptionInfo = static_cast<EXCEPTION_POINTERS*>(param);
                    logCrashInfo(exceptionInfo);

                    HANDLE file = CreateFileA("fomod_plus_stack_overflow.dmp", GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
                    ...
                    if (file != INVALID_HANDLE_VALUE) {
                        ...
                        MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), file, MiniDumpNormal, &dumpInfo,
                            nullptr, nullptr);
                        CloseHandle(file);
            ...
            HANDLE file = CreateFileA(
                "fomod_plus_crash.dmp", GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
            ...
            if (file != INVALID_HANDLE_VALUE) {
                ...
                MiniDumpWriteDump(
                    GetCurrentProcess(), GetCurrentProcessId(), file, MiniDumpNormal, &dumpInfo, nullptr, nullptr);
                CloseHandle(file);
